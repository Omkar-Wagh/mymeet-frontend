...
type TabSyncMessage = {
  type: "CLAIM" | "STATE" | "COMMAND"
  roomId: string
  participantId: string
  tabId: string
  action?: "MIC" | "CAMERA" | "HAND" | "SCREEN_SHARE" | "REACTION"
  value?: boolean
  emoji?: string
  micOn?: boolean
  cameraOn?: boolean
...
export default function MeetingRoom({
  ...
  const tabCommandHandlerRef =
    useRef<((message: TabSyncMessage) => void) | null>(null)
  ...
  const broadcastTabMessage = useCallback(
    (message: Omit<TabSyncMessage, "roomId" | "participantId" | "tabId">) => {
      const channel = tabChannelRef.current
      if (!channel || !tabIdRef.current || !participantIdRef.current) return
      try {
        channel.postMessage({ ...message, roomId, participantId: participantIdRef.current, tabId: tabIdRef.current } satisfies TabSyncMessage)
      } catch {
        // Ignore local-tab synchronization failures.
      }
    },
    [roomId]
  )
  ...
  useEffect(() => {
    if (typeof window === "undefined" || !tabIdRef.current || !("BroadcastChannel" in window)) return

    const channel = new BroadcastChannel(`mymeet:participant:${roomId}:${participantIdRef.current}`)
    tabChannelRef.current = channel

    const handleMessage = (event: MessageEvent<TabSyncMessage>) => {
      const message = event.data
      if (!message || message.roomId !== roomId || message.participantId !== participantIdRef.current || message.tabId === tabIdRef.current) return
      ...
      if (message.type === "STATE" && message.tabId === activeTabIdRef.current) {
        if (message.micOn !== undefined) setMicOn(message.micOn)
        if (message.cameraOn !== undefined) setCameraOn(message.cameraOn)
        if (message.handRaised !== undefined) setHandRaised(message.handRaised)
        if (message.screenSharing !== undefined) setIsScreenSharing(message.screenSharing)
        setParticipants((current) => current.map((p) => p.id === participantIdRef.current ? { ...p, name: message.name?.trim() || p.name, muted: message.micOn !== undefined ? !message.micOn : p.muted, cameraOff: message.cameraOn !== undefined ? !message.cameraOn : p.cameraOff, handRaised: message.handRaised ?? p.handRaised, screenSharing: message.screenSharing ?? p.screenSharing } : p))
      }
    }
  ...
  useEffect(() => {
    if (joinPhase !== "ready" || !isActiveTab) return
  ...
  const triggerReactionDisplay =
    useCallback(
      (
        ...
        const previousTimer = reactionTimersRef.current.get(participantId)
  ...
  const join = useCallback(
    async () => {
      ...
      try {
        ...
        activeTabIdRef.current = tabIdRef.current
        setIsActiveTab(true)
        broadcastTabMessage({ type: "CLAIM" })
  ...
  useEffect(() => {
    return () => {
      ...
      if (tabChannelRef.current) {
        try { tabChannelRef.current.close() } catch { /* Ignore. */ }
  ...
  const toggleMic =
    useCallback(() => {
      ...
      if (!isActiveTab) {
        broadcastTabMessage({ type: "COMMAND", action: "MIC", value: next })
        return
      }
      ...
      broadcastTabMessage({ type: "STATE", micOn: next, cameraOn, handRaised, screenSharing: isScreenSharing, name: nameRef.current })
    }, [
      broadcastTabMessage,
      cameraOn,
      handRaised,
  ...
  const toggleCamera =
    useCallback(() => {
      ...
      if (!isActiveTab) {
        broadcastTabMessage({ type: "COMMAND", action: "CAMERA", value: next })
        return
      }
      ...
      broadcastTabMessage({ type: "STATE", micOn, cameraOn: next, handRaised, screenSharing: isScreenSharing, name: nameRef.current })
    }, [
      broadcastTabMessage,
      cameraOn,
      handRaised,
  ...
  const toggleScreenShare =
    useCallback(async () => {
      if (!isActiveTab) {
        broadcastTabMessage({ type: "COMMAND", action: "SCREEN_SHARE", value: !isScreenSharing })
        return
      }
      ...
      if (isScreenSharing) {
        ...
        broadcastTabMessage({ type: "STATE", micOn, cameraOn, handRaised, screenSharing: false, name: nameRef.current })
      ...
      try {
        ...
        broadcastTabMessage({ type: "STATE", micOn, cameraOn, handRaised, screenSharing: true, name: nameRef.current })
  ...
  const toggleHandRaise =
    useCallback(() => {
      ...
      if (!isActiveTab) {
        broadcastTabMessage({ type: "COMMAND", action: "HAND", value: nextState })
        return
      }
      ...
      broadcastTabMessage({ type: "STATE", micOn, cameraOn, handRaised: nextState, screenSharing: isScreenSharing, name: nameRef.current })
    }, [
      broadcastTabMessage,
      cameraOn,
      handRaised,
  ...
  const sendReaction =
    useCallback(
      (emoji: string) => {
        ...
        if (!isActiveTab) {
          broadcastTabMessage({ type: "COMMAND", action: "REACTION", emoji })
          return
        }
  ...
  useEffect(() => {
    tabCommandHandlerRef.current = (message) => {
      if (message.action === "MIC" && message.value !== undefined) {
        const next = message.value
        localStreamRef.current?.getAudioTracks().forEach((track) => { track.enabled = next })
        setMicOn(next)
        setParticipants((current) => current.map((p) => p.id === participantIdRef.current ? { ...p, muted: !next } : p))
        publish("/app/meet/media-status", { roomId, participantId: participantIdRef.current, muted: !next, cameraOff: !cameraOn })
        broadcastTabMessage({ type: "STATE", micOn: next, cameraOn, handRaised, screenSharing: isScreenSharing, name: nameRef.current })
        return
      }
      if (message.action === "CAMERA" && message.value !== undefined) {
        const next = message.value
        localStreamRef.current?.getVideoTracks().forEach((track) => { track.enabled = next })
        setCameraOn(next)
        setParticipants((current) => current.map((p) => p.id === participantIdRef.current ? { ...p, cameraOff: !next } : p))
        publish("/app/meet/media-status", { roomId, participantId: participantIdRef.current, muted: !micOn, cameraOff: !next })
        broadcastTabMessage({ type: "STATE", micOn, cameraOn: next, handRaised, screenSharing: isScreenSharing, name: nameRef.current })
        return
      }
      if (message.action === "HAND" && message.value !== undefined) {
        const next = message.value
        setHandRaised(next)
        setParticipants((current) => current.map((p) => p.id === participantIdRef.current ? { ...p, handRaised: next } : p))
        publish("/app/meet/hand-raise", { roomId, participantId: participantIdRef.current, handRaised: next })
        broadcastTabMessage({ type: "STATE", micOn, cameraOn, handRaised: next, screenSharing: isScreenSharing, name: nameRef.current })
        return
      }
      if (message.action === "SCREEN_SHARE" && message.value !== undefined && message.value !== isScreenSharing) {
        void toggleScreenShare()
        return
      }
      if (message.action === "REACTION" && message.emoji) {
        triggerReactionDisplay(participantIdRef.current, message.emoji)
      }
    }

    return () => { tabCommandHandlerRef.current = null }
  }, [broadcastTabMessage, cameraOn, handRaised, isScreenSharing, micOn, publish, roomId, toggleScreenShare, triggerReactionDisplay])
